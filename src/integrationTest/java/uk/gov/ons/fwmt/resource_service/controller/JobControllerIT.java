...
public class JobControllerIT {
  ...
  public void updateJobIT() throws Exception {
    ...
    jobEntity.setLastAuthNo("1234");
    jobRepo.save(jobEntity);
    mockMvc.perform((put("/jobs")).contentType(MediaType.APPLICATION_JSON).content(JOB_JSON).with(httpBasic("user", "password")));
  ...
  public void deleteJobIT() throws Exception {
    ...
    jobRepo.save(jobEntity);
    mockMvc.perform(delete("/jobs").contentType(MediaType.APPLICATION_JSON).content(JOB_JSON).with(httpBasic("user", "password")));
  ...
  public void createJobIT() throws Exception {
    mockMvc.perform(post("/jobs").contentType(MediaType.APPLICATION_JSON).content(JOB_JSON).with(httpBasic("user", "password")));
